"""Phase-7 ledger snapshot with explicit forecast-proposal dispatch.

Execution/accounting equations are unchanged; tests enforce source-body parity.
The original package stays untouched so existing evidence remains replayable.
"""
...
from portfolio_fx_risk.sizing import size
...
def run(rows: tuple[Candle,...], rates: tuple[Rate,...], model: str, start, end,
        ...
        fx_capital_fraction: float=.10, proposal_factory: Callable=propose) -> dict:
    ...
    for row in test:
        ...
        if row.end.hour%4==0 and row.end.minute==0:
            pending=proposal_factory(rows,rates,row.end,model,Capacity(nav,nav*fx_capital_fraction),risk,volatility,config,execution)
